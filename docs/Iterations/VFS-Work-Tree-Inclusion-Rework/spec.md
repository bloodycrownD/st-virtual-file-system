...
| [`register-vfs-macros.ts`](../../../src/infra/sillytarvern/macros/register-vfs-macros.ts) | `VIRTUAL_WORK_TREE` 读 `chatVfsSnapshot` + `workTree` | 仅消费新 `WorkTreeConfig` 形状（或 `workTree` 为 `null` 时宏为空字符串，行为保持）。 |
