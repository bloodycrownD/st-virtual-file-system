...
- 明确“即时刷新”为主目标，同时将当前分支既有创建失败语义纳入可接受护栏范围。
...
- 失败路径沿用当前分支基线实现，不在本迭代新增校验或错误映射语义。
- 创建失败时继续按既有行为处理；该行为被视为本迭代护栏，可在测试中显式断言“不回归”。
- 即时刷新仍是本迭代主目标，失败语义仅作为边界稳定性约束。
...
src/app/screens/business-screens/
  VfsMainScreen.vue         # 创建成功后刷新时序修复主入口（失败路径语义按当前分支基线接受）

test/
  vfs-ui-cr-loop.spec.ts    # 新建后即时可见回归用例（失败行为断言允许验证基线语义不回归）
```
...
- **TC-5 失败路径不变**
  - 非法名称/冲突/异常仍维持当前分支基线报错、映射与状态；允许显式断言该基线不回归。
